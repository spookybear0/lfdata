...
class VisualElementGenerator:
    ...
    def __init__(
        ...
        self.hit_borders: list[LFHitBorderInstance] = []
        self._detect_camera_shakes()
        self._detect_hit_borders()
            ...
            if et in ('0205', '0206', '0207', '0208'):
                ...
                if et == '0306':
                    ...
                    if actor_id == self.entity_id:
                        ...
                        if (
                            found_lock
                            and found_lock.target_id == self.entity_id
                        ):
    ...
    def _build_variables_blob(
        self,
        game_time_ms: int,
        players: dict[str, LFReplayPlayerState],
    ) -> dict[str, str]:
        """Builds a dictionary of text variables for Jinja rendering.

        Args:
            game_time_ms: The current game time in milliseconds.
            players: The dictionary of player states.
    ...
    def _render_jinja_text(
        self,
        template_str: str,
        local_vars: dict[str, str],
    ) -> str:
        """Renders a Jinja template string using the local variables cache.

        Args:
            template_str: The Jinja template string to render.
            local_vars: The variables dictionary.

        Returns:
            str: The rendered text string.
        """
        cache_key = (template_str, frozenset(local_vars.items()))
        if cache_key in self._jinja_cache:
            return self._jinja_cache[cache_key]
    ...
    def _get_active_multiline_lines(
        ...
        for ev in sorted_events:
            ...
            if slot_idx != -1:
                text: str = ev.desc
                target_color_override = getattr(
                    ev, 'target_color_override', None
                )
                if getattr(
                    ev, 'double_resup_desc', None
                ) is not None and time_ms >= getattr(
                    ev, 'double_resup_time', 0
                ):
    ...
    def _detect_hit_borders(self) -> None:
        """Precomputes hit border instances for the current player."""
        if not self.entity_id:
            return
        ...
        hb_config = self.config.get('elements', {}).get('hit_border', {})
        enabled = hb_config.get('enabled', True)
        if not enabled:
            return

        dur_hp_s = hb_config.get('duration_hp_s', 0.5)
        dur_down_s = hb_config.get('duration_down_s', 1.0)
        max_scale = hb_config.get('max_scale', 1.2)
        ...
        for event in self.game.events:
            ...
            is_zap = event_type in ('0205', '0206', '0207', '0208')
            ...
            hp = 1
            for snap_time, players, _ in self.snapshots:
                if snap_time == event.time:
                    p_state = players.get(self.entity_id)
                    if p_state:
                        hp = p_state.hp
                        break
            ...
            if is_resupply:
                ...
                tint_hex = color_other

            self.hit_borders.append(
                LFHitBorderInstance(
                    start_ms=event.time,
                    duration_ms=dur_ms,
                    tint_hex=tint_hex,
